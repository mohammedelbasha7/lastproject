...
class AuthService:
    ...
    async def get_or_create_user(self, platform_sub: str, email: str, name: Optional[str] = None) -> User:
        ...
        is_configured_admin = (
            bool(email)
            and bool(getattr(settings, "admin_user_email", ""))
            and email.lower() == settings.admin_user_email.lower()
        ) or (
            bool(platform_sub)
            and bool(getattr(settings, "admin_user_id", ""))
            and str(platform_sub) == str(settings.admin_user_id)
        )
