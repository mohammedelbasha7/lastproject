...
async def validate_id_token(id_token: str, access_token: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            issuer_options = [issuer]
            if issuer.rstrip("/") == "https://accounts.google.com":
                issuer_options.append("accounts.google.com")

            last_claims_error = None
            payload = None
            for issuer_option in issuer_options:
                try:
                    payload = jwt.decode(
                        id_token,
                        pem_key,
                        algorithms=["RS256"],
                        issuer=issuer_option,
            ...
            if payload is None:
                raise last_claims_error or JWTClaimsError("Token claims validation failed")
...
def build_authorization_url(
    ...
    if not authorization_url:
        if settings.oidc_issuer_url.rstrip("/") == "https://accounts.google.com":
            ...
            authorization_url = f"{settings.oidc_issuer_url.rstrip('/')}/authorize"
